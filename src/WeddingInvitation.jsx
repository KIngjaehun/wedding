...
const WEDDING_CONFIG = {
  ...
  // 갤러리 이미지 - URL을 교체하세요
  gallery: [
    "https://firebasestorage.googleapis.com/v0/b/wedding-invitation-e1f08.firebasestorage.app/o/IMG_3190.jpeg?alt=media&token=5f8b3a69-7a53-4b04-9870-5375cda6748a",
  ...
  const openKakaoMap = () => {
    window.open(
      "https://map.kakao.com/link/search/" +
        encodeURIComponent(WEDDING_CONFIG.wedding.address),
      "_blank"
    );
  };
...
END:VCALENDAR`;
  ...
  const renderCalendar = () => {
    const [year, month, day] = WEDDING_CONFIG.wedding.date
      .split("-")
      .map(Number);
    ...
    <div style={styles.container}>
          ...
          <button
            ...
            onClick={() =>
              setCurrentImageIndex(
                (currentImageIndex - 1 + WEDDING_CONFIG.gallery.length) %
      ...
      <section style={styles.calendarSection}>
        ...
        <p style={styles.calendarDate}>
          ...
          {
            ["일", "월", "화", "수", "목", "금", "토"][
              new Date(WEDDING_CONFIG.wedding.date).getDay()
            ]
          }
          요일
        </p>
        ...
        <div style={styles.calendarGrid}>
          ...
          <div style={styles.calendarBody}>{renderCalendar()}</div>
        </div>
        ...
        <div style={styles.transportInfo}>
          <div style={styles.transportItem}>
            ...
            <p style={styles.transportText}>
              {WEDDING_CONFIG.transport.subway}
            </p>
          </div>
          <div style={styles.transportItem}>
        ...
        <div style={styles.accountCards}>
          {activeSection === 0 ? (
            <>
              <div style={styles.accountCard}>
                <p style={styles.accountLabel}>
                  신랑 {WEDDING_CONFIG.groom.account.holder}
                </p>
                <p style={styles.accountNumber}>
                  {WEDDING_CONFIG.groom.account.bank}{" "}
                  {WEDDING_CONFIG.groom.account.number}
                </p>
                <button
                  onClick={() => copyAccount(WEDDING_CONFIG.groom.account)}
                  style={styles.copyBtn}
                >
                  {copySuccess === WEDDING_CONFIG.groom.account.holder
                    ? "복사완료 ✓"
                    : "복사하기"}
                </button>
              </div>
              <div style={styles.accountCard}>
                <p style={styles.accountLabel}>
                  신랑 아버지 {WEDDING_CONFIG.groom.fatherAccount.holder}
                </p>
                <p style={styles.accountNumber}>
                  {WEDDING_CONFIG.groom.fatherAccount.bank}{" "}
                  {WEDDING_CONFIG.groom.fatherAccount.number}
                </p>
                <button
                  onClick={() =>
                    copyAccount(WEDDING_CONFIG.groom.fatherAccount)
                  }
                  style={styles.copyBtn}
                >
                  {copySuccess === WEDDING_CONFIG.groom.fatherAccount.holder
                    ? "복사완료 ✓"
                    : "복사하기"}
                </button>
              </div>
            ...
            <>
              <div style={styles.accountCard}>
                <p style={styles.accountLabel}>
                  신부 {WEDDING_CONFIG.bride.account.holder}
                </p>
                <p style={styles.accountNumber}>
                  {WEDDING_CONFIG.bride.account.bank}{" "}
                  {WEDDING_CONFIG.bride.account.number}
                </p>
                <button
                  onClick={() => copyAccount(WEDDING_CONFIG.bride.account)}
                  style={styles.copyBtn}
                >
                  {copySuccess === WEDDING_CONFIG.bride.account.holder
                    ? "복사완료 ✓"
                    : "복사하기"}
                </button>
              </div>
              <div style={styles.accountCard}>
                <p style={styles.accountLabel}>
                  신부 아버지 {WEDDING_CONFIG.bride.fatherAccount.holder}
                </p>
                <p style={styles.accountNumber}>
                  {WEDDING_CONFIG.bride.fatherAccount.bank}{" "}
                  {WEDDING_CONFIG.bride.fatherAccount.number}
                </p>
                <button
                  onClick={() =>
                    copyAccount(WEDDING_CONFIG.bride.fatherAccount)
                  }
                  style={styles.copyBtn}
                >
                  {copySuccess === WEDDING_CONFIG.bride.fatherAccount.holder
                    ? "복사완료 ✓"
                    : "복사하기"}
                </button>
              </div>
              ...
              <div key={entry.id} style={styles.guestbookEntry}>
                <div style={styles.entryHeader}>
                  ...
                  <div style={styles.entryActions}>
                    ...
                    <button
                      onClick={() =>
                        deleteGuestbookEntry(entry.id, entry.password)
                      }
      ...
      {copySuccess && copySuccess !== "URL" && copySuccess !== "주소" && (
        <div style={styles.toast}>{copySuccess} 계좌번호 복사 완료!</div>
      )}
    </div>
  );
};
